...
def train_local(i, epoch, batch, id_1, id_2, losses):
        ...
        y_transform = T.Resize(size = (4,4))
        y_valid = y_transform(valid)
        z_transform = T.Resize(size = (2,2))
        z_valid = z_transform(valid)
        loss_GAN_1_Y = criterion_GAN(D[id_1](fake_Y1),y_valid)
        loss_GAN_1_Z = criterion_GAN(D[id_1](fake_Z1),z_valid)

        loss_GAN_1 = (loss_GAN_1_X+loss_GAN_1_Y+loss_GAN_1_Z) /3
        
        loss_GAN_2_X = lambda_0 * criterion_GAN(D[id_2](fake_X2), valid)
        loss_GAN_2_Y = criterion_GAN(D[id_2](fake_Y2),y_valid)
        loss_GAN_2_Z = criterion_GAN(D[id_2](fake_Z2),z_valid)

        loss_GAN_2 = (loss_GAN_2_X + loss_GAN_2_Y + loss_GAN_2_Z)/3
