...
def train_local(i, epoch, batch, id_1, id_2, losses):
        ...
        # Reconstruct speech
        recon_X1,recon_Y1,recon_Z1,recon_Y1_down,recon_Z1_down = G[id_1](Z1)
        recon_X2,recon_Y2,recon_Z2,recon_Y2_down,recon_Z2_down = G[id_2](Z2)



        # Translate speech
        fake_X1,fake_Y1,fake_Z1,fake_Y1_down,fake_Z1_down = G[id_1](Z2)
        fake_X2,fake_Y2,fake_Z2,fake_Y2_down,fake_Z2_down = G[id_2](Z1)
        ...
        cycle_X1,cycle_Y1,cycle_Z1,_,_ = G[id_1](Z2_)
        cycle_X2,cycle_Y2,cycle_Z2,_,_ = G[id_2](Z1_)

        # Losses
        loss_GAN_1_X =criterion_GAN(D[id_1](fake_X1), valid)
        scale_transform = T.Resize(size = (8,8))
        loss_GAN_1_Y = criterion_GAN(scale_transform(D[id_1](fake_Y1)),valid)
        loss_GAN_1_Z = criterion_GAN(scale_transform(D[id_1](fake_Z1)),valid)
        loss_GAN_1_Y_down = criterion_GAN(scale_transform(D[id_1](fake_Y1_down)),valid)
        loss_GAN_1_Z_down = criterion_GAN(scale_transform(D[id_1](fake_Z1_down)),valid)
        
        weighted_loss_GAN_1_x = 0.5*(loss_GAN_1_X)
        weighted_loss_GAN_1_y = 0.125*(loss_GAN_1_Y)
        weighted_loss_GAN_1_z = 0.125*(loss_GAN_1_Z)
        weighted_loss_GAN_1_y_down = 0.125*(loss_GAN_1_Y_down)
        weighted_loss_GAN_1_z_down = 0.125*(loss_GAN_1_Z_down)
        

        loss_GAN_1 = lambda_0 * (weighted_loss_GAN_1_x+weighted_loss_GAN_1_y+weighted_loss_GAN_1_z+weighted_loss_GAN_1_z_down+weighted_loss_GAN_1_y_down)
        
        loss_GAN_2_X = criterion_GAN(D[id_2](fake_X2), valid)
        loss_GAN_2_Y = criterion_GAN(scale_transform(D[id_2](fake_Y2)),valid)
        loss_GAN_2_Z = criterion_GAN(scale_transform(D[id_2](fake_Z2)),valid)
        loss_GAN_2_Y_down = criterion_GAN(scale_transform(D[id_2](fake_Y2_down)),valid)
        loss_GAN_2_Z_down = criterion_GAN(scale_transform(D[id_2](fake_Z2_down)),valid)
        
        
        weighted_loss_GAN_2_x = 0.5*(loss_GAN_2_X)
        weighted_loss_GAN_2_y = 0.125*(loss_GAN_2_Y)
        weighted_loss_GAN_2_z = 0.125*(loss_GAN_2_Z)
        weighted_loss_GAN_2_y_down = 0.125*(loss_GAN_2_Y_down)
        weighted_loss_GAN_2_z_down = 0.125*(loss_GAN_2_Z_down)

        loss_GAN_2 = lambda_0 * (weighted_loss_GAN_2_x+weighted_loss_GAN_2_y+weighted_loss_GAN_2_z+weighted_loss_GAN_2_y_down+weighted_loss_GAN_2_z_down)
        ...
        generator_loss = loss_G.item() - weighted_loss_GAN_1_y - weighted_loss_GAN_1_z -weighted_loss_GAN_2_y-weighted_loss_GAN_2_z
        losses['G'].append(loss_G.item())
        losses['G_N'].append(generator_loss)
